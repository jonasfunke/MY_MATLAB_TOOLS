...
for i=2:length(filenames)
    data(i) = load_fcs_data_attune(pathname, filenames{i}, path_out, 0.05, data(1).roi_position);
...
for j=1:length(filenames)
        sample_names{j} = [ filenames{j}(12:end-4) ' ' data(j).fcshdr.par(ch_R1).name];
end
...
N_channel = 12;%size(data(j).fcsdat,2)/2-1;

legend_tmp = {};

for j=1:length(filenames)
    
    for i=2:13%2:2:size(data(j).fcsdat,2)-2
        subplot(N_channel, 1, i-1)
...
for j=1:length(filenames)
    ...
    scatter(data(j).fcsdat(:,i_fsc_ch),data(j).fcsdat(:,i_ssc_ch), 5, data(j).fcsdat(:,i_fl_ch), '.'), hold on
    h = colorbar;
    ylabel(h, data(j).fcshdr.par(i_fl_ch).name)

...
RGB = zeros(size(img_bf,1), size(img_bf,2),3, 22, 'uint8');
...
for i= 1:length(files_bf) %[1:7 9:10] %
    ...
    RGB(:,:,1,i) = uint8((bf_norm/1)*(2^8-1));
    RGB(:,:,2,i) = uint8((bf_norm/1 + (fl_norm-0.1)/1)*(2^8-1));
    RGB(:,:,3,i) = uint8((bf_norm/1)*(2^8-1));
 
    %imshow(RGB)
    %pause
end
%% fits write
imwrite(RGB, [path_out files_bf(i).name(1:end-7) '_RGB.TIF']);

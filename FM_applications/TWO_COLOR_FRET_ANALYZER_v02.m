...
sigma_threshold = 5; % usually = 2
ratio_thresholds = [0 2]; % usually = [0.2, 1.2]

criteria = ones(N_peaks_raw,4 );
criteria(:,1:2) = filter_spots(dd_fit_raw(:,3:4), ratio_thresholds, sigma_threshold);
criteria(:,3:4) = filter_spots(aa_fit_raw(:,3:4), ratio_thresholds, sigma_threshold);
...
close all
fig_dim =1*[20 10];
cur_fig = figure('Visible','off', 'PaperPositionMode', 'manual','PaperUnits','centimeters','PaperPosition', [0 0 fig_dim(1) fig_dim(2)], 'Position', [0 1000 1000 500]);
colormap gray
w_plot = 10;
...
w_plot = 6;
close all
cur_fig = figure(); %figure('Visible','on', 'PaperPositionMode', 'auto','PaperUnits','points','PaperPosition', [0 0 20 10], 'Position', [250 500 1000 500 ]);
...
for i=1:size(traces,1)
    ...
    plot(traces{i,1}(:,1), traces{i,1}(:,4), 'g-', ...
